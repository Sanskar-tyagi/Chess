...
public class Queen  extends Piece {
    ...
    private final static int[] CANDIDATE_MOVES_VECTOR_COORDINATES={-9,-8,-7,-1,1,7,8,9}; //algorithm to check all valid Moves
    ...
    public Collection<Move> calculateLegalMoves(final Board board) {
        ...
        for (final int candidateCoordinateOffSet : CANDIDATE_MOVES_VECTOR_COORDINATES) {
            ...
            while (BoardUtils.IsValidTileCoordinate(candidateDestinationCoordinate)) {
                if (isFirstColumnException(candidateDestinationCoordinate, candidateCoordinateOffSet) || isEightColumnException(candidateDestinationCoordinate, candidateCoordinateOffSet)) {
                    break;
                }
    ...
    private static boolean isFirstColumnException(int CurrentPosition, final int candidateOffset) {
        return BoardUtils.First_Column[CurrentPosition] && (candidateOffset==-1 || candidateOffset==9||  candidateOffset == -9 || candidateOffset == 7);
    }
